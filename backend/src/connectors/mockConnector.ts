...
function ts(daysAgo: number, hour = 10): string {
  const d = new Date();
...
function mockEntities(): Entity[] {
  ...
  for (const t of teams) list.push({ ...t, type: 'team', source: SOURCE_ID });
  for (const p of people) list.push({ ...p, type: 'person', source: SOURCE_ID });
  for (const p of projects) list.push({ ...p, type: 'project', source: SOURCE_ID });
  for (const c of channels) list.push({ ...c, type: 'channel', source: SOURCE_ID });
  for (const t of topics) list.push({ ...t, type: 'topic', source: SOURCE_ID });
  return list;
}
...
function mockActivities(): Activity[] {
  ...
  const tasks: Array<{ title: string; body: string; author: string; links: string[]; daysAgo: number }> = [
...
function mockRelationships(): Relationship[] {
