...
function buildGraph() {
    ...
    const relationships = store_1.store.getAllRelationships();
    ...
    for (const a of activities) {
        const ids = [...new Set([...a.linkedEntityIds, a.author].filter(Boolean))];
        for (let i = 0; i < ids.length; i++) {
            for (let j = i + 1; j < ids.length; j++) {
                if (!nodeMap.has(ids[i]) || !nodeMap.has(ids[j]))
                    continue;
