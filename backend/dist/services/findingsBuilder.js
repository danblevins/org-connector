...
function buildFindings() {
    ...
    for (const a of activities) {
        ...
        for (const pid of linkedProjects) {
            if (!projectTeams.has(pid))
                projectTeams.set(pid, new Set());
        ...
        if (author) {
            if (!personTeams.has(author))
                personTeams.set(author, new Set());
    ...
    for (const [projectId, teams] of projectTeams) {
        ...
        if (teamOnly.size >= 2) {
            ...
            const count = activities.filter((a) => a.linkedEntityIds.includes(projectId) || a.author).length;
            findings.push({
                id: `cross-${projectId}`,
                type: 'cross_team',
